...
Validation_Size = 20000; % Validation dataset Size
K = 512; % number of subcarriers
miniBatchSize  = 50; % Size of the minibatch for the Deep Learning
% Note: The axes of the antennas match the axes of the ray-tracing scenario
...
%     jx = zeros(u_step,M,size(at_codebook,2));
    for u=1:u_step:params.num_user                        
        for uu=1:1:u_step
            ...
            DL_input(:,u+uu-1+((pp-1)*params.num_user))= reshape([real(G) imag(G)].',[],1); % first dimension: 2*256, second dimension : 18100
...
for dd=1:1:numel(Training_Size)
    ...
    %--------------------YTrain reshaping---------------------------%
    YTrain = reshape(sumrate(Training_Ind,:),1,1,size(sumrate(Training_Ind,:),2),size(sumrate(Training_Ind,:),1));
%     beamusedy=unique(Y);
...
%     beamYTrain = (tabulate(YYYTrain));
%     YTrain = categorical(YYYTrain);  
    %-----------------------XTrain reshaping-----------------------%
    X = DL_input(:,Training_Ind);
%     DL_input_reshaped = cat(2,X,zeros([M*K_DL*2,size(bny,1)]));
    XTrain = reshape(X,size(X,1),1,1,size(X,2));
    %---------------------YValidation reshaping----------------------%
    YValidation = reshape(sumrate(Validation_Ind,:),1,1,size(sumrate(Validation_Ind,:),2),size(sumrate(Validation_Ind,:),1));
...
%     YValidation = categorical(cat(1,YValidation,bnyv));
    ...
    XVal = (DL_input(:,Validation_Ind));
%     DL_val_reshaped = cat(2,XVal,zeros([M*K_DL*2,size(bnyv,1)]));
    XValidation = reshape(XVal,size(XVal,1),1,1,size(XVal,2));
   %---------------------NN classess & weights----------------------%
    classes = categorical(1:1:M*at_Nbeams);
%     classweights = (beamYTrain(:,3))+0.0001;


    yuni = unique(YTrain);yvuni = unique(YValidation);
...
%     XValidation= reshape(XXValidation,size(XXValidation,1),1,1,size(XXValidation,2));XVal_size = size(XValidation);
    ...
    clear XVal X Y max_beam DL_input_reshaped DL_val_reshaped DL_output_un
    clear VI_rev_sortind VI_sortind RandP_all
    layers = [
        imageInputLayer([size(XTrain,1),1,1],'Name','input')
        fullyConnectedLayer(size(XTrain,1),'Name','Fully1')
        reluLayer('Name','relu1')
        dropoutLayer(0.5,'Name','dropout1')
        fullyConnectedLayer(size(XTrain,1),'Name','Fully2')
        reluLayer('Name','relu2')
        dropoutLayer(0.5,'Name','dropout2')
        fullyConnectedLayer(size(XTrain,1),'Name','Fully3')
...
%     MaxR_OPT = single(zeros(numel(Indmax_OPT),1));                      
    ...
    % --------------------- Achievable Rate --------------------------%                    
     ...
     for i = 1:1:Validation_Size
         for j =1:1:size(YPredicted,2)
     top1rate(i) = sumrate(Validation_Ind(i),top10beams(j,1));
     top5rate(i) = sum(sumrate(Validation_Ind(i),top10beams(j,1:5)))/5;
     top10rate(i) = sum(sumrate(Validation_Ind(i),top10beams(j,1:10)))/10;
         end
         optimalrate(i) = maxrate(Validation_Ind(i));
     end
     topavg1 = 1-mean(top1rate)/mean(optimalrate);
     topavg5 = 1-mean(top5rate)/mean(optimalrate);
     topavg10 = 1-mean(top10rate)/mean(optimalrate);
     

     top1acc = 0;top5acc = 0;top10acc = 0;
     accloss = top10beams-repmat(double(YValidation),1,10);
     for i = 1:1:Validation_Size
        if accloss(i,1) ==0
            top1acc = top1acc+1;
        end
        if prod(accloss(i,1:5),2) ==0
            top5acc = top5acc+1;
        end
        if prod(accloss(i,1:10),2) ==0
            top10acc = top10acc+1;
        end
     end
top1acc = top1acc/Validation_Size;top5acc = top5acc/Validation_Size;top10acc = top10acc/Validation_Size;                                      
